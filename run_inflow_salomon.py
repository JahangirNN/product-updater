"""
Nordstrom Salomon Shoes Full Ingestion Pipeline
Ingests discovered Salomon shoes from Nordstrom into the partitioned JSON database.
Enforces constraint: Adult Footwear (All Available Sizes: min_size_variants=1).
Stores all available sizes with appropriate UK sizes alongside US sizes using the official Salomon size conversion matrix.
...
def process_and_save_product(
    ...
    if not canonical:
        # Excluded (e.g. kids category leak or < min_size_variants)
        return None
...
def run_full_inflow(max_workers: int = 3, min_size_variants: int = 1):
    print("=" * 75)
    print("NORDSTROM SALOMON SHOES CATALOG INGESTION (X-MODE TIER 1)")
    ...
    urls_file = "scratch/all_salomon_urls.json"
    ...
    total = len(urls)
    print(f"Loaded {total} unique Nordstrom Salomon shoe URLs for ingestion.")
