...
| **Centralized Delta Configuration** | [`config/delta_config.json`](../../config/delta_config.json) | 120m check interval, concurrency, polite delays, and per-store profiles |
...
| **Local JSON Database** | [`docs/JSON_STORAGE_SPEC.md`](../../docs/JSON_STORAGE_SPEC.md) | Partitioned file layout (`db/{store}/products/{id}.json`), `index.json`, `delta_events.json` queue |
...
| **Architecture Decision Records** | [`docs/adr/`](../../docs/adr/) | Numbered, immutable logs of technical decisions (0001 through 0008) |
...
## 2. The 5 Golden Rules of Project Maintenance
...
2. **Document Decisions in ADRs**: If a new architectural choice, library addition, or schema migration is proposed, create a new numbered record in `docs/adr/` (e.g. `0009-title.md`) before writing implementation code.
...
5. **Prioritize Ingestion & Freshness First**: Maintain laser focus on rich catalog attribute collection and systematic 2-hour delta freshness. Downstream Shopify Admin API syncing is decoupled and deferred to future phases.
...
1. Find the next sequential number in `docs/adr/` (e.g., `0009`).
...
3. Implement pure functions in `stores/{new_store_slug}/inflow.py` and `delta.py` (`check_price_and_stock`, `apply_delta_to_product`).
4. Register the new store in `config/delta_config.json` with appropriate concurrency and rate limits.
5. Run validation to verify zero-duplicate storage, systematic delta freshness, and Shopify readiness.
